...
module OmfRcCloud
  module Node
    ...
    # Set up a timer to check for reclaimable children
    #
    hook :before_ready do |res|
      OmfCommon.eventloop.every(5) do
        res.reclaim_children
      end
    ...
    # Check if any of the created resources can be released. This assumes
    # that reclaimable resources implement "work 'reclaimable?'"
    #
    work 'reclaim_children' do |res|
      res.children.find_all { |v| v.respond_to? :reclaimable? }.each do |v|
        debug "Check child #{v}: #{v.reclaimable?}"
        if v.reclaimable?
          log "Release #{v}"
          res.release(v.uid)
        end
